...
var app = {
  apikey: "709896649beff87110e50624654c47e8004e7ceb",
  map: L.map('map', { center: [40.75583970971843, 113.795166015625], zoom: 3 }),
  geojsonClient: new cartodb.SQL({ user: 'wenhaowuuu', format: 'geojson' }),
...
app.geojsonClient.execute("SELECT * FROM economicindicator_chinesecities_1") // 'LIMIT' should be added to the end of this line
  .done(function(data) {
    console.log('first wave of points');
    layerPlotted = L.geoJson(data, {
...
// select only those data that have gdp larer than 6000
$('#filter').click(function(){
  if(layerPlotted){
    app.map.removeLayer(layerPlotted);
    // console.log(layerPlotted);
  }

  app.geojsonClient.execute("SELECT * FROM economicindicator_chinesecities_1 as ec WHERE ec.gdp > 6000 ") // 'LIMIT' should be added to the end of this line
    .done(function(data) {
      console.log(data);
      layerPlotted = L.geoJson(data, {
        onEachFeature: function(feature, layer) {
    ...
    console.log('second wave of points');
});
